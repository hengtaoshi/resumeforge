...
const Toggle: React.FC<{ active: boolean; onClick: () => void }> = ({ active, onClick }) => (
  <button onClick={onClick} className={`relative w-9 h-5 rounded-full transition-colors shrink-0 ${active ? 'bg-brand-500' : 'bg-slate-200'}`}>
...
const Settings: React.FC = () => {
  ...
  return (
    <div className="flex-1 p-4 overflow-y-auto bg-[#F8F7F4] h-full">
      <div className="mb-3">
        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-500 mb-2">
      ...
      <div className="grid grid-cols-2 gap-3 max-w-4xl">
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-3 col-span-2">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <select value={model} onChange={e => setModel(e.target.value)}
              className="text-sm px-2 py-1.5 rounded-md border border-slate-200 bg-white text-slate-700">
            ...
            <div className="relative flex-1 min-w-[200px] max-w-xs">
              <input type={showKey ? 'text' : 'password'} value={currentKey || ''}
                ...
                className="w-full text-sm px-2.5 py-1.5 rounded-md border border-slate-200 bg-white text-slate-700 placeholder:text-slate-300 outline-none focus:border-brand-300"/>
              <button onClick={() => setShowKey(!showKey)} className="absolute right-2 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500">
                <i className={`ph-light text-sm ${showKey ? 'ph-eye-slash' : 'ph-eye'}`} />
              </button>
            </div>
            <button onClick={testConn} disabled={!currentKey || testing}
              className="text-sm px-2.5 py-1.5 rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50 disabled:opacity-40 shrink-0">
              {testing ? '测试中...' : '测试连接'}
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => window.electronAPI?.openExternal(API_KEY_URLS[aiProvider])}
              className="text-[11px] px-2 py-1 rounded-md bg-brand-50 text-brand-600 hover:bg-brand-100 border-none cursor-pointer transition-colors shrink-0 ml-2">
              获取 Key ↗
            </button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600">界面语言</span>
              <select value={language} onChange={e => setLanguage(e.target.value)}
                className="text-sm px-2 py-1 rounded-md border border-slate-200 bg-white text-slate-600">
                <option value="zh-CN">简体中文</option>
                <option value="en-US">English</option>
              </select>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600">默认模板</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600">自动保存</span>
              <Toggle active={autoSave} onClick={() => setAutoSave(!autoSave)} />
            </div>
            <div className="flex items-center justify-between">
              <span className="text-sm text-slate-600">PDF导出页数</span>
