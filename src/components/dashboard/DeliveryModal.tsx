  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl w-[520px] max-h-[80vh] flex flex-col mx-4" onClick={e => e.stopPropagation()}>
        ...
        {/* Form */}
        <form onSubmit={(e) => { e.preventDefault(); handleAdd(); }} className="px-6 pt-4 pb-3 border-b border-slate-100 space-y-3">
          ...
          <input value={url} onChange={e => setUrl(e.target.value)} placeholder="投递链接（选填）" maxLength={500}
            className="w-full px-3 py-2 rounded-lg border border-slate-200 text-sm outline-none focus:border-accent transition-colors" />
