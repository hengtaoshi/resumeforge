...
const SortableSectionItem = ({ section, deleteMode, isSelected, onToggleSelect, onToggleVisibility }: SortableSectionItemProps) => {
  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.5 : 1 }}
      className={`flex items-center gap-2 px-3 py-2.5 rounded-lg cursor-default transition-colors ${deleteMode && isSelected ? 'bg-red-50 ring-1 ring-red-200' : isDragging ? 'bg-gray-100 shadow-md' : 'hover:bg-gray-50'}`}>
      <button {...attributes} {...listeners} className="text-gray-400 hover:text-gray-600 cursor-grab active:cursor-grabbing"><i className="ph-light ph-list text-base"></i></button>
...
export default function EditorSidebar(props: SidebarProps) {
  ...
  return (
    <div className="border-r bg-white flex flex-col shrink-0" style={{ width: props.width || 260 }}>
      <div className="flex items-center justify-between px-4 py-3 border-b">
        <div className="flex items-center gap-2"><h3 className="font-semibold text-gray-800 text-sm">版块列表</h3></div>
        <button onClick={() => { setDeleteMode(!deleteMode); setSelectedForDelete([]) }}
          className={`p-1.5 rounded-lg transition-colors ${deleteMode ? 'bg-red-100 text-red-600' : 'text-gray-400 hover:bg-gray-100'}`} title={deleteMode ? '退出删除模式' : '删除版块'}>
          <TrashIcon size={16} />
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-2">
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={sortedSections.map(s => s.id)} strategy={verticalListSortingStrategy}>
            <div className="space-y-1">{sortedSections.map(section => (
