...
const ICONS: Record<string, React.ReactNode> = {
  ...
  interview: <><path d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></>,
...
export default function Sidebar({ activePage, onPageChange }: SidebarProps) {
  ...
  return (
    <nav className="flex flex-col items-center w-[64px] h-full bg-white border-r border-slate-200 py-3 gap-1 z-10 shadow-sm">
      {Object.entries(ICONS).map(([key, svg]) => {
        const isActive = activePage === key;
        return (
          <div key={key} className="relative w-full flex justify-center group">
            {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-6 bg-sky-500 rounded-r-full" />}
            <button
              onClick={() => handleClick(key as PageKey)}
              className={`relative flex items-center justify-center w-10 h-10 rounded-xl transition-all duration-150 ease-in-out ${isActive ? 'bg-sky-100 text-sky-600 shadow-sm' : 'text-slate-400 hover:bg-slate-200/60 hover:text-slate-600'}`}
              aria-label={key}
            >
            ...
            <div className="absolute left-full ml-3 px-2.5 py-1 bg-slate-800 text-white text-xs rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-150 whitespace-nowrap pointer-events-none z-50">
              {({ dashboard: '概览', editor: '打造简历', tiers: '生成简历', jd: 'JD匹配', chat: 'AI助手', tools: '求职工具', kanban: '求职看板', scanner: '职位扫描', interview: '面试技巧', settings: '设置' } as Record<string, string>)[key]}
